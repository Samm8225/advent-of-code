...
fun String.transformInputIntoPasswords(): List<Password> {
    val lines = this.split("\n")
    return lines
        .map { it.split(" ") } // collection where first element is min-max, second is char, and third is pass
        .map {
            val min = it[0].split("-")[0].toInt()
            val max = it[0].split("-")[1].toInt()
            val charboi = it[1].split(":")[0].toCharArray()[0]
            val pass = it[2]
            Password(min, max, charboi, pass)
        }
}

fun getInput2(): String {
    return """1-8 n: dpwpmhknmnlglhjtrbpx
